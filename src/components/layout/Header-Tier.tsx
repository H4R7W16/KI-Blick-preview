...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--color-border)] dark:bg-[var(--color-surface)] bg-white/80 backdrop-blur-sm">
      <div className="flex items-center justify-between h-14 px-4">
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-1.5 p-2 rounded-lg border transition-colors ${leftButtonClass}`}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[var(--color-card)] dark:text-[var(--color-secondary)] text-slate-600 transition-colors"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 p-2 rounded-lg border transition-colors ${rightButtonClass}`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`relative flex-1 py-2 text-center text-xs font-medium transition-colors no-underline ${
                isActive
